package com.rcloud.server.whiteboard.sms;
...
public class SmsTemplateService {
    ...
    public void postConstruct() {
        try {
            ...
            List<SmsTemplateVO> smsTemplateVOList = objectMapper
                .readValue(jsonData, new TypeReference<List<SmsTemplateVO>>() {
                });
        ...
        try {
            ...
            ObjectMapper objectMapper = new ObjectMapper();
            List<RegionLanguageVO> regionLanguageVOList = objectMapper
                .readValue(jsonData2, new TypeReference<List<RegionLanguageVO>>() {
                });
    ...
    public String getLangByRegion(String region) {
        ...
        if (StringUtils.isBlank(lang)) {
            //为空，返回 sms_template_map.json 默认配置语言
            lang = regionLanguageMap.get("global_default_language");
            if (StringUtils.isBlank(lang)) {
                //默认配置为空，返回中文
                lang = "zh_cn";
            }
